import { IconHeart } from '@tabler/icons-react';
import {
  Card,
  Image,
  Text,
  Group,
  Badge,
  Button,
  ActionIcon,
  createStyles,
  rem,
} from '@mantine/core';
import useBlog from '../../hooks/useBlog';
import {  useParams } from 'react-router';



const useStyles = createStyles((theme) => ({
  card: {
    backgroundColor: theme.colorScheme === 'dark' ? theme.colors.dark[7] : theme.white,
    
  },

  section: {
    borderBottom: `${rem(1)} solid ${
      theme.colorScheme === 'dark' ? theme.colors.dark[4] : theme.colors.gray[3]
    }`,
    paddingLeft: theme.spacing.md,
    paddingRight: theme.spacing.md,
    paddingBottom: theme.spacing.md,
  },

  like: {
    color: theme.colors.red[6],
  },

  label: {
    textTransform: 'uppercase',
    fontSize: theme.fontSizes.xs,
    fontWeight: 700,
  },
}));
 

const  BlogDetails= () =>{
  const {blog , isLoading} = useBlog()
  const { id } = useParams()
  const { classes, theme } = useStyles();
  
  const blogs = blog?.filter((temp) => temp._id === id)
  
 if(blogs === undefined)
   {
    return null
   }
 
  const { title , body , tag , blogImage } =  {...blogs[0]}
 
  
        
  
  const image ='https://encrypted-tbn0.gstatic.com/images?q=tbn:ANd9GcTWRij83RCMCEYA5i6wCZCe0H6Hhy640RS8yw&usqp=CAU'



  const features =blogs !== undefined && tag?.map((badge,index) => (
    <Badge
      color={theme.colorScheme === 'dark' ? 'dark' : 'gray'}
      key={badge.index}
      leftSection={badge.emoji}
    >
      {badge.value}
    </Badge>
  )); 

 

  return (
    <>{isLoading? <h2>..isLoding</h2> : <Card withBorder radius="md" p="md" style={{width:'80%', marginTop:'2rem' }} className={classes.card}>
    <Card.Section>
      <Image src={blogImage !==undefined ? blogImage: image} alt={title}  height={180} />
    </Card.Section>

    <Card.Section className={classes.section} mt="md">
      <Group position="apart">
        <Text fz="lg" fw={500}>
          {title}
        </Text>
                  {tag?.map((temp)=>

                      temp!==undefined?
                      <Badge color="blue" variant="light" ml='xs'>{temp}</Badge>
                      :<Badge color="pink" variant="light"> known </Badge>
                          
                 )}
      </Group>
      <Text fz="sm" mt="xs" width={200}>
        {body}
      </Text>
    </Card.Section>

    <Card.Section className={classes.section}>
      <Text mt="md" className={classes.label} c="dimmed">
        Perfect for you, if you enjoy
      </Text>
      <Group spacing={7} mt={5}>
        {features}
      </Group>
    </Card.Section>

    <Group mt="xs">
      <Button radius="md" style={{ flex: 1 }}>
        Show details
      </Button>
      <ActionIcon variant="default" radius="md" size={36}>
        <IconHeart size="1.1rem" className={classes.like} stroke={1.5} />
      </ActionIcon>
    </Group>
  </Card> }
    
    
    </>
     
  );
}

export default  BlogDetails